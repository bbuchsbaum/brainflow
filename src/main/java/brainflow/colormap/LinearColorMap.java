 ...
 */
public final class LinearColorMap extends AbstractColorMap {
    ...
    public LinearColorMap(double min, double max, double lowClip, double highClip, IndexColorModel icm) {
        assert max > min : "max must exceed min in LinearColorMap2";
    ...
    public LinearColorMap newClipRange(double lowClip, double highClip) {
        if (lowClip > highClip) lowClip = highClip;
        else if (highClip < lowClip) highClip = lowClip;

        assert highClip >= lowClip;
        return new LinearColorMap(getMinimumValue(), getMaximumValue(), lowClip, highClip, this);
    }

     public LinearColorMap newClipRange(double lowClip, double highClip, double min, double max) {
        if (lowClip > highClip) lowClip = highClip;
        else if (highClip < lowClip) highClip = lowClip;
        ...
        assert highClip >= lowClip;
        return new LinearColorMap(min, max, lowClip, highClip, this);
    }
}
