...
public class PixelUtils {
    ...
    /**
     * Clamp a file to the range 0..255
     */
    ...
    public static Kernel makeKernel(float radius, float xspacing, float yspacing) {
        ...
        int center = (int) Math.floor(rows / 2.0);
        ...
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < cols; col++) {
                float distance = (float) Math.sqrt(Math.pow((row - center) * xspacing, 2) + Math.pow((col - center) * yspacing, 2));
                matrix[row * cols + col] = (float) Math.exp(-(distance) / sigma22) / sqrtSigmaPi2;
                sum = sum + matrix[row * cols + col];
            }
        }

        float sf = 1f / sum;
        for (int i = 0; i < matrix.length; i++) {
            matrix[i] = sf * matrix[i];
        }


        return new Kernel(rows, cols, matrix);

    }

    public static void main(String[] args) {
        Kernel k = PixelUtils.makeKernel(2, 2.4f, 2.4f);

        float[] tmp = new float[k.getWidth() * k.getHeight()];
        float[] ret = k.getKernelData(tmp);
        System.out.println("kernel " + Arrays.toString(ret));
