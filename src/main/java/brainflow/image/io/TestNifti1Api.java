...
public class TestNifti1Api {
    ...
    public static void main(String[] args) {
        ...
        if (args[0].equals("print")) {
            if (args.length != 2) {
                System.out.println("\nError, print command takes exactly zero parameter: print <dataset name>");
                return;
            }
        ...
        if (args[0].equals("copy")) {
            if (args.length != 3) {
                System.out.println("\nError, copy command takes exactly one parameters: copy <dataset name> <new dataset name>");
                return;
            }
            Nifti1Dataset nds = new Nifti1Dataset(args[1]);
            try {
        ...
        System.out.println("\three\tB -- each volume has voxels set from 0 to n, where n=(X*Y*Z)-1");
        System.out.println("\three\tX -- voxels are 0");

        System.out.println("\textend <dataset> <extension_file> <code>");
        System.out.println("\three\ttake <extension_file> and stuff it into the header of <dataset>\n\three\tas an extension of type <code>");
        System.out.println("\three\three<code> is an identifying code for the extension type (ask Bob).");
