 ...
 */
    public void readHeader(InputStream inputStream) throws IOException, FileNotFoundException {
        ...
        if (ds_hdrname.endsWith(".gz"))
            dis = new MemoryCacheImageInputStream(new GZIPInputStream(inputStream));
        else
        ...
        try {
            ...
            if ((s < 1) || (s > 7)) {
                ...
                big_endian = true;
                dis.setByteOrder(ByteOrder.BIG_ENDIAN);

            }
            dis.seek(0);


            sizeof_hdr = dis.readInt();

            bb = new byte[10];
            dis.readFully(bb, 0, 10);
            data_type_string = new StringBuffer(new String(bb));

            bb = new byte[18];
            dis.readFully(bb, 0, 18);
            db_name = new StringBuffer(new String(bb));

            extents = dis.readInt();

            session_error = dis.readShort();

            regular = new StringBuffer();
            regular.append((char) (dis.readUnsignedByte()));

            dim_info = new StringBuffer();
            dim_info.append((char) (dis.readUnsignedByte()));
            ss = unpackDimInfo((int) dim_info.charAt(0));
            freq_dim = ss[0];
            phase_dim = ss[1];
            slice_dim = ss[2];
            ...
            for (i = 0; i < 3; i++)
                intent[i] = dis.readFloat();

            intent_code = dis.readShort();

            datatype = dis.readShort();

            bitpix = dis.readShort();

            slice_start = dis.readShort();

            for (i = 0; i < 8; i++)
                pixdim[i] = dis.readFloat();
            qfac = (short) Math.floor((double) (pixdim[0]));

            vox_offset = dis.readFloat();

            scl_slope = dis.readFloat();
            scl_inter = dis.readFloat();

            slice_end = dis.readShort();

            slice_code = (byte) dis.readUnsignedByte();

            xyzt_units = (byte) dis.readUnsignedByte();
            ss = unpackUnits((int) xyzt_units);
            xyz_unit_code = ss[0];
            t_unit_code = ss[1];

            cal_max = dis.readFloat();
            cal_min = dis.readFloat();

            slice_duration = dis.readFloat();

            toffset = dis.readFloat();

            glmax = dis.readInt();
            glmin = dis.readInt();

            bb = new byte[80];
            dis.readFully(bb, 0, 80);
            descrip = new StringBuffer(new String(bb));

            bb = new byte[24];
            dis.readFully(bb, 0, 24);
            aux_file = new StringBuffer(new String(bb));

            qform_code = dis.readShort();
            sform_code = dis.readShort();

            for (i = 0; i < 3; i++)
                quatern[i] = dis.readFloat();
            for (i = 0; i < 3; i++)
                qoffset[i] = dis.readFloat();

            for (i = 0; i < 4; i++)
                srow_x[i] = dis.readFloat();
            for (i = 0; i < 4; i++)
                srow_y[i] = dis.readFloat();
            for (i = 0; i < 4; i++)
                srow_z[i] = dis.readFloat();


            bb = new byte[16];
            dis.readFully(bb, 0, 16);
            intent_name = new StringBuffer(new String(bb));

            bb = new byte[4];
            dis.readFully(bb, 0, 4);
            magic = new StringBuffer(new String(bb));

        }
        catch (IOException ex) {
            throw new IOException("Error: unable to read header file " + ds_hdrname + ": " + ex.getMessage(), ex);
        }
...
////
    ...
    */
    ...
    private void readNp1Ext(ImageInputStream dis) throws IOException, EOFException {
