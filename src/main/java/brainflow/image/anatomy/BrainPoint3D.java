 ...
 */
public class BrainPoint3D implements BrainPoint {
    ...
    private BrainPoint1D z;

    private Anatomy3D anatomy;

   
    public BrainPoint3D(Anatomy3D _anatomy, double x, double y, double z) {
        anatomy = _anatomy;

        this.x = new BrainPoint1D(anatomy.XAXIS, x);
        this.y = new BrainPoint1D(anatomy.YAXIS, y);
        this.z = new BrainPoint1D(anatomy.ZAXIS, z);

    }



    public BrainPoint3D snapToBounds(IImageSpace3D space) {
       ...
       double newx = x.getValue();
        ...
        if (z.getValue() < space.getImageAxis(Axis.Z_AXIS).getMinimum()) {
            ...
            BrainPoint3D ret = new BrainPoint3D(space.getAnatomy(), newx, newy, newz);
            return ret;
    ...
    public BrainPoint3D replace(BrainPoint1D pt) {
        if (pt.getAnatomy() == x.getAnatomy()) {
            return new BrainPoint3D(getAnatomy(), pt.getValue(), y.getValue(), z.getValue());
        } else if (pt.getAnatomy() == y.getAnatomy()) {
            return new BrainPoint3D(getAnatomy(), x.getValue(), pt.getValue(), z.getValue());
        } else if (pt.getAnatomy() == z.getAnatomy()) {
            return new BrainPoint3D(getAnatomy(), x.getValue(), y.getValue(), pt.getValue());
        } else {
            throw new IllegalArgumentException("supplied point has incomaptible axis : " + pt.getAnatomy());
        }
    }

    public BrainPoint3D convertTo(ICoordinateSpace3D otherSpace) {

        //todo what happens if "other" is not contained by "this"?

        BrainPoint1D to_x = getValue(otherSpace.getAnatomy().XAXIS);
        BrainPoint1D to_y = getValue(otherSpace.getAnatomy().YAXIS);
        BrainPoint1D to_z = getValue(otherSpace.getAnatomy().ZAXIS);

        BrainPoint1D retx = to_x.convertTo(otherSpace.getImageAxis(Axis.X_AXIS));
        BrainPoint1D rety = to_y.convertTo(otherSpace.getImageAxis(Axis.Y_AXIS));
        BrainPoint1D retz = to_z.convertTo(otherSpace.getImageAxis(Axis.Z_AXIS));


        return new BrainPoint3D(otherSpace.getAnatomy(), retx.getValue(), rety.getValue(), retz.getValue());


    }

    public static BrainPoint3D convertToWorld(BrainPoint3D pt, IImageSpace3D space) {
        //todo this conversion seems like a hack ...
        pt = pt.convertTo(space);

        if (space.getAnatomy() != pt.getAnatomy()) {
            throw new IllegalArgumentException("BrainPoint3D and ImageSpace arguments must have same anatomical orientation: " +
        ...
        pt = new BrainPoint3D(Anatomy3D.REFERENCE_ANATOMY, ret[0], ret[1], ret[2]);
        return pt;

    }

    public static BrainPoint3D convertFromWorld(BrainPoint3D worldpt, IImageSpace3D space) {
        ...
        return new BrainPoint3D(space.getAnatomy(), x1, y1, z1);
    }
   ...
   /* public BrainPoint1D getValue(ImageAxis axis) {
    ...
    public BrainPoint1D getValue(AnatomicalAxis axis, double min, double max) {
        if (axis.sameAxis(getAnatomy().XAXIS)) {
            return new BrainPoint1D(axis, getAnatomy().XAXIS.convertValue(axis, min, max, x.getValue()));
        } else if (axis.sameAxis(getAnatomy().YAXIS)) {
            return new BrainPoint1D(axis, getAnatomy().YAXIS.convertValue(axis, min, max, y.getValue()));
        } else if (axis.sameAxis(getAnatomy().ZAXIS)) {
            return new BrainPoint1D(axis, getAnatomy().ZAXIS.convertValue(axis, min, max, z.getValue()));
        } else {
            throw new IllegalArgumentException("illegal axis " + axis + " for this coordinate space");
        }
    } 
    ...
    public boolean equals(Object o) {
        ...
        BrainPoint3D that = (BrainPoint3D) o;
    ...
    public static void main(String[] args) {
        BrainPoint3D a = new BrainPoint3D(Anatomy3D.AXIAL_LAI, 12, 50, 12);
        BrainPoint3D b = new BrainPoint3D(Anatomy3D.AXIAL_LPI, 12, 50, 12);
